...
class StencilGradients(nn.Module):
    ...
    def __init__(self, d0=1, d1=1, fd_acc = 2, periodic=False, device = 'cpu'):
        ...
        # opt15a (DEAD): wrapping the stencils in torch.compile is incompatible with the rest of the
        # stack — the residual path reaches them through both the compiled model and functorch
        # (jacfwd/vmap → FX symbolic trace), and "FX-tracing a dynamo-optimized function" is
        # unsupported (crashes at iter 0). Kernel-count reduction is instead done eagerly via
        # channel-batched convs in ResidualsDarcy.compute_residual (opt15b).
